...
{
	static t_readbuffer	readbuffers[FD_MAX];
	...
	while (readbuffer_read(&readbuffers[fd], fd))
	{
		has_eol = readbuffer_has_eol(&readbuffers[fd], &buffer_lineend);
		if (!varstr_add(&result_varstr, readbuffers[fd].linestart,
				buffer_lineend))
			return (readbuffer_updatestart(&readbuffers[fd], NULL),
				varstr_free(&result_varstr));
		if (has_eol)
			return (readbuffer_updatestart(&readbuffers[fd], buffer_lineend),
				result_varstr.str);
		readbuffer_updatestart(&readbuffers[fd], NULL);
	}
